;(function () {
    ...
    if (canPromise) {
        cadesplugin = new Promise((resolve, reject) => {
    ...
    const cpcsp_console_log = (level, msg) => {
        ...
        if (level <= cadesplugin.current_log_level) {
            if (level === cadesplugin.LOG_LEVEL_DEBUG) console.log(`DEBUG: ${msg}`);
            if (level === cadesplugin.LOG_LEVEL_INFO) console.info(`INFO: ${msg}`);
            if (level === cadesplugin.LOG_LEVEL_ERROR) console.error(`ERROR: ${msg}`);
        }
    }
    
    const set_log_level = (level) => {
        if (![cadesplugin.LOG_LEVEL_DEBUG, cadesplugin.LOG_LEVEL_INFO, cadesplugin.LOG_LEVEL_ERROR].includes(level)) {
